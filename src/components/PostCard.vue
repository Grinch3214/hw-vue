<template>
  <div class="post-card">
      ...
      >
          ...
          <p v-if="readMore" class="post-card__card-body">{{ formatedCommentText(post.body) }}</p>
          <p v-else class="post-card__card-body">{{ post.body }}</p>

          <el-button  @click="readMore =! readMore" type="text">Read more</el-button>
  ...
  data() {
      return {
          ...
          post: {
              userId: 1,
              id: 2,
              title: 'Sunt post card aut facere repellat provident occaecati excepturi optio reprehenderit',
